...
void LCD_Init(void)
{
  ...
  LCD_WR_REG(0x2A); LCD_WR_DATA(0); LCD_WR_DATA(0);
  LCD_WR_DATA((LCD_WIDTH-1)>>8); LCD_WR_DATA((LCD_WIDTH-1)&0xFF);
  LCD_WR_REG(0x2B); LCD_WR_DATA(0); LCD_WR_DATA(0);
  LCD_WR_DATA((LCD_HEIGHT-1)>>8); LCD_WR_DATA((LCD_HEIGHT-1)&0xFF);
  LCD_WR_REG(0x2C);
  for (uint32_t i = 0; i < (uint32_t)LCD_WIDTH * LCD_HEIGHT; i++) { LCD_WR_DATA(0); }
  Delay_ms(120);
